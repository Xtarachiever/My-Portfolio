...
const generateEmailContent = (data) => {
  ...
  const htmlData = Object.entries(data).reduce((str, [key, val]) => (str += `<h3>${CONTACT_MESSAGE_FIELDS[key]}</h3><p>${val}</p>`), "");
  return {
    text: stringData,
    html: `<!doctype html>
        <html>
        ...
        <table width=100% border=0 cellspacing=0 cellpadding=0>
        <tr>
        <td style=padding:0;font-size:16px;line-height:25px;color:#232323>
  ...
  try {
    ...
    if (!email || !subject || !message)
      return NextResponse.json(
        { error: "Don't have a complete form data" },
        { status: 422 }
      );
    ...
    // if(emailCheck) return NextResponse.json({ error: "Email Already exists" },{status:400});

    const contactData = await Contact.create({ email, subject, message });
    return NextResponse.json({ data: contactData }, { status: 201 });
  } catch (error) {
